Here is an explenation of the different pieces of code in this project.
...
Explenation on the different pieces of code in order of running:

	- preproc10: first code to run. 
	  Starts with loading an enviroment using setupEnviroment11(), which loads the paths, loads fieldtrip etc and general variables.
	  the data is loaded using load_data12() which contains the variables for loading data from each dataset (paths, file type etc.)
	  First you do a general prepcessing (demean, detrend, filters...).
	...
	- freq_analysis20: second code to run. 
	  ...
	  Then, you can compute the LAVI and create a (or add to an existing) dataframe containing the LAVI calculations of all the participants 
	  using create_datArr21().
	  You can either calculate the LAVI of specific participants and add it to an existing dataframe by putting cfg.prev = 'add',
 ...
 * user_func: This function contains the name of the user and the letter of the DataAyelet folder on your computer. 
   ...
   This is instead of changing it within the coee each time, allowing for more comfortable sharing of code using github.
